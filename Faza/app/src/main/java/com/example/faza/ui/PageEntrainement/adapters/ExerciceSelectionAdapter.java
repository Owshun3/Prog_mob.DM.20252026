...
public class ExerciceSelectionAdapter extends RecyclerView.Adapter<ExerciceSelectionAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder h, int position) {
        ...
        String info;
        if (!gp.isEmpty() && !gs.isEmpty()) info = gp + " / " + gs;
        else if (!gp.isEmpty()) info = gp;
        else if (!gs.isEmpty()) info = gs;
        else info = "";
        h.txtGroupes.setText(info);
    ...
    public List<String> getGroupesPrincipaux() {
        ...
        for (Exercice e : items) {
            ...
            if (g != null && !g.trim().isEmpty() && !groupes.contains(g))
                groupes.add(g);
        }
